...
public final class DiskLruCache implements Closeable, Flushable {
    ...
    private final Runnable cleanupRunnable = new Runnable() { // from class: neton.internal.cache.DiskLruCache.1
    ...
    public static DiskLruCache create(FileSystem fileSystem, File file, int i, int i2, long j) {
        if (j <= 0) {
            throw new NetonException(new IllegalArgumentException("maxSize <= 0"));
        }
        if (i2 <= 0) {
            throw new NetonException(new IllegalArgumentException("valueCount <= 0"));
        }
    ...
    private BufferedSink newJournalWriter() {
        return Okio.buffer(new FaultHidingSink(this.fileSystem.appendingSink(this.journalFile)) { // from class: neton.internal.cache.DiskLruCache.2
            static final /* synthetic */ boolean $assertionsDisabled = false;

            @Override // neton.internal.cache.FaultHidingSink
    ...
    final synchronized Editor edit(String str, long j) {
        initialize();
        checkNotClosed();
        validateKey(str);
        Entry entry = this.lruEntries.get(str);
    ...
    final synchronized void completeEdit(Editor editor, boolean z) {
        ...
        if (z && !entry.readable) {
            for (int i = 0; i < this.valueCount; i++) {
                if (!editor.written[i]) {
                    editor.abort();
                    throw new NetonException(new IllegalStateException("Newly created entry didn't create value for index ".concat(String.valueOf(i))));
                }
    ...
    final boolean removeEntry(Entry entry) {
        if (entry.currentEditor != null) {
    ...
    private synchronized void checkNotClosed() {
        if (isClosed()) {
            throw new NetonException(new IllegalStateException("cache is closed"));
        }
    }
    ...
    private void validateKey(String str) {
        ...
        throw new NetonException(new IllegalArgumentException("keys must match regex [a-z0-9_-]{1,120}: \"" + str + "\""));
    }

    public final synchronized Iterator<Snapshot> snapshots() {
        initialize();
        return new Iterator<Snapshot>() { // from class: neton.internal.cache.DiskLruCache.3
    ...
    public final class Editor {
        ...
        public final Sink newSink(int i) {
            synchronized (DiskLruCache.this) {
                ...
                try {
                    return new FaultHidingSink(DiskLruCache.this.fileSystem.sink(this.entry.dirtyFiles[i])) { // from class: neton.internal.cache.DiskLruCache.Editor.1
                        @Override // neton.internal.cache.FaultHidingSink
                        protected void onException(IOException iOException) {
                            synchronized (DiskLruCache.this) {
                                Editor.this.detach();
